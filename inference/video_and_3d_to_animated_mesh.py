...
@torch.no_grad()
def run_actionmesh(
    pipeline: ActionMeshPipelineWithMeshInput,
    input: str,
    mesh_input: str,
    output_dir: str,
    seed: int,
    ...
    anchor_mesh = load_glb(mesh_input)
    anchor_visual = anchor_mesh.visual.copy()
    original_faces = anchor_mesh.faces.copy()
    ...
    if check_blender_available(blender_path):
        ...
        # Reconstruct textured anchor mesh from the expanded vertices
        anchor_for_export = trimesh.Trimesh(
        ...
        with tempfile.NamedTemporaryFile(suffix=".glb", delete=False) as tmp:
            anchor_for_export.export(tmp.name, file_type="glb")
            tmp_glb_path = tmp.name

        create_animated_glb(
            vertices_npy=str(vertices_path),
            faces_npy=str(faces_path),
            output_glb=animated_glb_path,
            blender_path=blender_path,
            fps=8,
            input_glb=tmp_glb_path,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the anchor mesh (.glb)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    pipeline: ActionMeshPipelineWithMeshInput = ActionMeshPipelineWithMeshInput(
